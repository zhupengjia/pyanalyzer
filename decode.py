...
class decode:
    def __init__(self,run):
      ...
      self.fodefname="%sHRS.odef"%self.arm[1]
    ...
    #create replay.C
    def buildreplayc(self,events=0):
      self.getvar()
      replayall=-1 if events<1 else 0
      with open(self.freplayc,"wb",-1) as f:
          ...
          f.write("void %s(Int_t run=%i,Int_t defAll=%i,Int_t defEvt=%i){\n"%(self.freplayname,self.run,replayall,events))
          ...
          f.write("  ReplayCore(run,defAll,defEvt,RNAME,ODEF,CUTS,%i,%i,0,1);\n"%(self.scaleron,self.helicityon))
          f.write("}\n")
    ...
    def replay(self,run=False,events=0):
      if not run:run=self.run
      self.buildreplayc(events)
      self.buildodef()
